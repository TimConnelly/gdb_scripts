"""
ASCII plotting version of the cv_imshow script.

This shows an image, but there's issues
1. adjust image size to terminal size
2. parse data range for ASCII usage

There maybe other TUI plotting libraries that do better 
with images.
"""
...
class cv_tuimshow(gdb.Command):
    """Diplays the content of an opencv image"""

    def __init__(self):
        super(cv_tuimshow, self).__init__('cv_tuimshow',
                                        gdb.COMMAND_SUPPORT,
                                        gdb.COMPLETE_SYMBOL)
    ...
    def show_image(width, height, n_channel, line_step, data_address, data_symbol):
        ...
        if n_channel == 1:
            ...
            img_list = img2list(img)
            plt.matrix_plot(img_list)
